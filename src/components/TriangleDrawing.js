...
const TriangleDrawing = ({ sides, angles }) => {
  ...
  const drawTriangle = () => {
    ...
    try {
      ...
      // Add side labels
      addSideLabel(vertices[1], vertices[2], 'a', sides[0]);
      addSideLabel(vertices[0], vertices[2], 'b', sides[1]);
      addSideLabel(vertices[0], vertices[1], 'c', sides[2]);
      ...
      // Add vertex labels
      addVertexLabel(vertices[0], 'A');
      addVertexLabel(vertices[1], 'B');
      addVertexLabel(vertices[2], 'C');
    } catch (error) {
